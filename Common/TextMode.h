...
#define TEXTMODE_BUFFER_TO_STREAM_NO_ENCODE(f) \
	((static_cast<BYTE>(f) & TEXTMODE_BUFFER_MASK) << 2)
#define TEXTMODE_STREAM_TO_BUFFER_NO_ENCODE(f) \
	((static_cast<BYTE>(f) & TEXTMODE_STREAM_MASK) >> 2)
...
#define TEXTMODE_IS_NO_CONVERTION(f) \
	(!static_cast<BYTE>(f) || (static_cast<BYTE>(f) & TEXTMODE_BUFFER_MASK) == TEXTMODE_STREAM_TO_BUFFER_NO_ENCODE(f))
